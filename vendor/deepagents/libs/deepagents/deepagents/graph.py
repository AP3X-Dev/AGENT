...
def create_deep_agent(
    ...
    *,
    ...
    summarization_middleware: AgentMiddleware | None | bool = None,
) -> CompiledStateGraph:
    """Create a deep agent.
    ...
    Args:
        ...
        summarization_middleware: Custom summarization middleware to use instead of the default.

            - `None` (default): Use the built-in `SummarizationMiddleware` with default settings.
            - `False`: Disable summarization middleware entirely.
            - `AgentMiddleware` instance: Use the provided middleware instead of the default.
    ...
    # Determine which summarization middleware to use
    # summarization_middleware can be: None (default), False (disabled), or a custom middleware
    if summarization_middleware is False:
        # Disabled - no summarization
        _main_summarization: AgentMiddleware | None = None
        _subagent_summarization: AgentMiddleware | None = None
    elif summarization_middleware is not None:
        # Custom middleware provided - use it for main agent, default for subagents
        _main_summarization = summarization_middleware
        _subagent_summarization = SummarizationMiddleware(
            model=model,
            backend=backend,
            trigger=trigger,
            keep=keep,
        ...
        # Default - create standard summarization middleware for both
        _main_summarization = SummarizationMiddleware(
            model=model,
            backend=backend,
            trigger=trigger,
            keep=keep,
    ...
    if _subagent_summarization is not None:
        subagent_middleware.append(_subagent_summarization)
    ...
    if _main_summarization is not None:
        deepagent_middleware.append(_main_summarization)
